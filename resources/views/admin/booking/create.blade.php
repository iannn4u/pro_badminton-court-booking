...
@section('main')
    ...
    <form action="/admin/booking" method="post" class="max-w-xl mt-10 space-y-5">
        ...
        <div>
            ...
            <input type="text" id="phoneNumber" name="phoneNumber" value="{{ old('phoneNumber') }}" autocomplete="off"
                class="block w-full p-2 text-slate-900 border border-slate-500 rounded-lg bg-white text-xs focus:ring-slate-500 focus:border-slate-500">
        </div>
        <div class="w-min">
        ...
        <div class="sm:flex-row flex-col">
            <h2 class="block mb-2 text-sm font-medium text-slate-900">Jam
                Booking</h2>
            <ul id="cardSlot" class="grid w-full grid-cols-3 gap-2 mt-2 px-5 opacity-50">
        ...
        <div class="flex items-center mb-4">
            <input id="member_booking" type="checkbox" value="true" name="member_booking"
                class="w-4 h-4 text-slate-600 bg-slate-100 border-slate-300 rounded focus:ring-slate-500 focus:ring-2">
            <label for="member_booking" class="ms-2 text-sm font-medium text-slate-900 dark:text-slate-300">Member</label>
        </div>
    ...
    <script>
        ...
        inputName.addEventListener('input', () => {
            ...
            fetch(`/dropdown/pelanggan`, {
                ...
                .then(data => {
                    if (data.length > 0) {
                        const listPelanggan = cardPelanggan.querySelector('ul');
                        ...
                        cardPelanggan.classList.add('absolute');
                    } else {
                        cardPelanggan.classList.add('hidden');
        ...
        const cardLapangan = document.querySelector("#lapangan-options");
        const inputLapangan = document.querySelectorAll("input[name='court_booking']");
        const timeInputs = document.querySelectorAll("input[name='time_booking[]']");
        ...
        inputTanggal.addEventListener("change", () => {
            ...
            if (tanggal) {
                cardLapangan.classList.remove("opacity-50", "pointer-events-none");
                inputLapangan.forEach(input => {
                    input.checked = false;
                });

                cardSlot.querySelectorAll("input[type='checkbox']").forEach(input => {
                    cardSlot.classList.add("opacity-50", "pointer-events-none");
                    input.checked = false;
                    input.disabled = true;
                    input.nextElementSibling.classList.remove(
                        "peer-checked:border-slate-600",
                        "peer-checked:text-white",
                        "peer-checked:bg-slate-700"
                    );
                    input.nextElementSibling.classList.add("bg-gray-400/30", "text-gray-400/80",
                        "border-gray-400");
                });
            } else {
                cardLapangan.classList.add("opacity-50", "pointer-events-none");
                inputLapangan.forEach(input => {
                    input.checked = false;
                });
                cardSlot.querySelectorAll("input[type='checkbox']").forEach(input => {
        ...
        function cekJadwal(inputLapangan, incrementOpen, incrementClose) {
            ...
            fetch(`/booking/cekSlot`, {
                ...
                <li>
                    ...
                    cardSlot.classList.remove("opacity-50", "pointer-events-none");
                })
